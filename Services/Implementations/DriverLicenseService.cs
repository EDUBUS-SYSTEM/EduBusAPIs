...
namespace Services.Implementations
{
	public class DriverLicenseService : IDriverLicenseService
	{
		...
		public async Task<DriverLicenseResponse> CreateDriverLicenseAsync(CreateDriverLicenseRequest request)
		{
			...
			// Check if driver already has a license (only allow first-time create)
			var existingLicenses = await _driverLicenseRepository.FindByConditionAsync(dl => dl.DriverId == request.DriverId);
		...
		public async Task<DriverLicenseResponse?> GetDriverLicenseByIdAsync(Guid id)
		{
